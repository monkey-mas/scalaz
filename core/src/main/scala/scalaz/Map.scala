...
sealed abstract class ==>>[A, B] {
  ...
  def unionWithKey(other: A ==>> B)(f: (A, B, B) => B)(implicit o: Order[A]): A ==>> B =
    mergeWithKey((a: A, b: B, c: B) => some(f(a, b, c)), (x: A ==>> B) => (x: A ==>> B), (x: A ==>> B) => (x: A ==>> B))(this, other)
  ...
  def differenceWithKey[C](other: A ==>> C, f: (A, B, C) => Option[B])(implicit o: Order[A]): A ==>> B =
    mergeWithKey(f, (x: A ==>> B) => x, (_: A ==>> C) => empty[A, B])(this, other)
  ...
  def intersectionWithKey[C, D](other: A ==>> C)(f: (A, B, C) => D)(implicit o: Order[A]): A ==>> D =
    mergeWithKey((a: A, b: B, c: C) => some(f(a, b, c)), (_: A ==>> B) => empty[A, D], (_: A ==>> C) => empty[A, D])(this, other)
...
object ==>> extends MapInstances {
  ...
  *
  ...
  -- When calling mergeWithKey(f, g1, g2)(a, b), a function combining two
  ...
  *  @param  b   second map
  */
  def mergeWithKey[A: Order, B, C, D](f: (A, B, C) => Option[D], g1: (A ==>> B) => (A ==>> D), g2: (A ==>> C) => (A ==>> D))
                                     (a: A ==>> B, b: A ==>> C)(implicit o: Order[A]): A ==>> D = {
